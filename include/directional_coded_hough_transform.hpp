    ...
    for (int c=1; c<(int)img_size_wh_[0]-1; c++)
      for (int r=1; r<(int)img_size_wh_[1]-1; r++)
        if (img_[r*(int)img_size_wh_[0]+c]) {
          ...
          // convolution result
          const int mat11 = (int)img_[ref_pt_idx];
          const int mat12 = (int)img_[ref_pt_idx+1];
          const int mat13 = (int)img_[ref_pt_idx+2];
          const int mat21 = (int)img_[ref_pt_idx+img_size_wh_[0]];
          const int mat23 = (int)img_[ref_pt_idx+img_size_wh_[0]+2];
          const int mat31 = (int)img_[ref_pt_idx+2*img_size_wh_[0]];
          const int mat32 = (int)img_[ref_pt_idx+2*img_size_wh_[0]+1];
          const int mat33 = (int)img_[ref_pt_idx+2*img_size_wh_[0]+2];
          ...
          for (auto th:theta_filter_) {
            ...
            this->vote_map_[PI_DIV*rho_c_shift+th]++;
          }
        }
  }
